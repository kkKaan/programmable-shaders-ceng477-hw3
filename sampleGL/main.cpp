...
void display()
{
	...
	for (int i = 0; i < 3; i++)
    {
		...
		if (bunny_x < position_x + 0.75f && bunny_x > position_x - 0.75f && -30.0f + box_position_z * speed * 0.7f >= -4.25f && -30.0f + box_position_z * speed * 0.7f <= -2.2f)
        {
    ...
    if (hit_red) renderText(text, 10.0f, 760.0f, 0.75f, glm::vec3(1.0f, 0.0f, 0.0f));
    else renderText(text, 10.0f, 760.0f, 0.75f, glm::vec3(1.0f, 1.0f, 0.0f));
}
